"""
В этом модуле находятся служебные функции необходимые для
реализации основных функций метода.
Эти функции используются для работы с z-преобразованием и свертки массивов.
"""
import numpy as np
...
def down_sample(array, sample_factor):
    """
        Производит операцию dounsample(стрелочка вниз)удаляет каждый третий
        элементв массиве.

        Работает как с одномерными так и с двумерными массивами. В случае
        двумерного удаляет каждый второй элемент и по строкам и по столбцам.
        Keyword arguments:
            array - одномерный или двумерный массив
            sample_factor - int > 0. определяет какой элемент будет удаляться.
                Обычно это 2^k, но необязательно. Можно удалить и каждый 3-ий.
    """
...
def GetH_h(pow_):
    """
        Возвращает вектор , соответствующий z-преобразованию H_H(z^pow_)
